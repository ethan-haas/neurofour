  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-10 px-4 py-10">
      ...
      <section className="flex flex-col items-center gap-4 text-center">
        ...
        <p className="max-w-xl text-sm text-[var(--ink-2)] sm:text-base">
        ...
        {state.status === 'success' && champion ? (
          <div className="mt-2 flex flex-wrap items-center justify-center gap-x-8 gap-y-3 rounded-xl border border-[var(--border)] bg-[var(--surface)] px-6 py-4">
            <div className="flex flex-col items-center">
              <span className="text-2xl font-bold text-[var(--ink)]">{formatBytes(champion.size_bytes)}</span>
              <span className="text-xs text-[var(--ink-muted-text)]">champion size</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-2xl font-bold text-[var(--ink)]">{formatPct(champion.optimality)}</span>
              <span className="text-xs text-[var(--ink-muted-text)]">optimality</span>
            </div>
            <div className="flex flex-col items-center">
              <span className="text-2xl font-bold text-[var(--ink)]">{champion.neurogolf_score.toFixed(1)}</span>
              <span className="text-xs text-[var(--ink-muted-text)]">NeuroFour score</span>
            </div>
          </div>
        ) : state.status === 'loading' ? (
          <StatusBanner kind="loading" title="Loading current champion…" />
        ) : null}
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        <pre
          className="overflow-x-auto rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-3 text-xs text-[var(--ink)]"
          tabIndex={0}
          role="region"
      ...
      <section className="flex flex-col gap-3">
        ...
        <p className="text-sm text-[var(--ink-2)]">
          ...
          budget is what makes "strength per byte" a fair fight: without a compute ceiling, an agent could just brute
          -force search deeper and deeper regardless of how small its stored weights are.
        </p>
      </section>
